...
parser.add_argument('-s', '--settings'  , type=str  , default='config_xgboost_TwoGlobalTracker_2017.txt',    help='config file')
...
_mass = args.mass
_tree = args.tree
_max_order = args.max_order
_unblind = args.unblind
...
filename = 'inputdata/T3MMiniTree_xgboost_setting1_2017UL.root'
...
tree = rootfile.Get(_tree)
_max_order = min(_max_order, tree.GetEntries()-2)

getattr(pdfs, 'import')(mass)

c_powerlaw = ROOT.RooRealVar("c_PowerLaw_{}".format(category), "", 1, 0, 10)
powerlaw = ROOT.RooGenericPdf("PowerLaw_{}".format(category), "TMath::Power(@0, @1)", ROOT.RooArgList(mass, c_powerlaw))
getattr(pdfs, 'import')(powerlaw)

exp_slope = ROOT.RooRealVar('exp_slope_{}'.format(category), 'exp_slope', 0, -10, 10)
expo = ROOT.RooExponential('Exponential_{}'.format(category), 'Exponential', mass, exp_slope)
...
for i in range(1, _max_order+1):
    c_polynomial = [ ROOT.RooRealVar('c_Polynomial_{}_{}{}'.format(category, i, j), 'c_Polynomial_{}_{}{}'.format(category, i, j), 0.01, -10, 10) for j in range(1,i+1)]
    PolynomialMap['Polynomial_{}_{}'.format(category, i)] = ROOT.RooPolynomial('Polynomial_{}_{}'.format(category, i), 'Polynomial_{}_{}'.format(category, i), mass, ROOT.RooArgList(*c_polynomial))
    getattr(pdfs, 'import')(PolynomialMap['Polynomial_{}_{}'.format(category, i)])
...
data = None
if _unblind:
    data = ROOT.RooDataSet('data', '', tree, variables, data_cut[category]+" && "+category_cuts[category]+" && "+\
        bdtcuts[category],"eventWeight")#.binnedClone('data')
else:
    data = ROOT.RooDataSet('data', '', tree, variables, data_cut[category]+" && "+category_cuts[category]+" && "+\
        bdtcuts[category]+" && ({} > {}|| {} <{}) ".format(_mass, mass_range_right[category][0], _mass, mass_range_left[category][1]),"eventWeight")#.binnedClone('data')
...
gofmax  = 0
bestfit = None
families = ['Exponential', 'PowerLaw', 'Polynomial']
...
leg.Draw("SAME");  
ctmp_sig.SaveAs("plots/MultiPdfcategory_%s_SplusB.png" % category);
...
can.Update()
can.Modified()
cat = ROOT.RooCategory("roomultipdf_cat_{}".format(category), "")

multipdf = ROOT.RooMultiPdf("multipdf", "", cat, envelope)
#cat.setIndex([envelope.at(i).GetName() for i in range(envelope.getSize())].index('Exponential_{}'.format(category)))
outerspace = ROOT.RooWorkspace('ospace')
getattr(outerspace, 'import')(envelope)
getattr(outerspace, 'import')(multipdf)

can.SaveAs("MultiPdfWorkspaces/2017_"+category+"_plot.pdf", "pdf")
outerspace.writeToFile("MultiPdfWorkspaces/2017_twoGlobalTracker_{}.root".format(category.split('_')[-1]))
